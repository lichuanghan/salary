...
import { batchImportAttendances, type BatchImportResult, type AttendanceImport } from '../hooks/api'
import { parseAttendanceExcelFile, generateAttendanceTemplate, type ParsedAttendance } from '../hooks/useExcel'
...
export function AttendanceImportModal({ yearMonth, onClose, onSuccess }: AttendanceImportModalProps) {
  ...
  const handleFileSelect = async (selectedFile: File) => {
    ...
    setResult(null)
    try {
      const data = await parseAttendanceExcelFile(selectedFile)
      // 限制最多500条
      if (data.length > 500) {
  ...
  const handleImport = async () => {
    ...
    try {
      // 过滤有效数据（无错误）
      const validData: AttendanceImport[] = parsedData
        .filter(p => !p.error)
        .map(p => ({
          employee_no: p.data.employee_no || '',
          name: p.data.name || '',
          year_month: p.data.year_month || yearMonth,
          work_days: p.data.work_days,
          normal_days: p.data.normal_days,
          sick_leave_days: p.data.sick_leave_days,
          late_count: p.data.late_count,
          early_leave_count: p.data.early_leave_count,
          overtime_hours: p.data.overtime_hours,
        }))
      ...
      const importResult = await batchImportAttendances(validData)
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content animate-scale" onClick={e => e.stopPropagation()} style={{ maxWidth: '900px' }}>
                      ...
                      <tbody>
                        {parsedData.slice(0, 10).map((item) => (
                          <tr key={item.row} style={{ color: item.error ? 'var(--color-danger)' : 'inherit' }}>
                            ...
                            <td>{item.data.year_month}</td>
                            <td style={{ textAlign: 'right' }}>{item.data.work_days}</td>
                            <td style={{ textAlign: 'right' }}>{item.data.normal_days}</td>
                            <td style={{ textAlign: 'right' }}>{item.data.sick_leave_days}</td>
                            <td style={{ textAlign: 'right' }}>{item.data.late_count}</td>
                            <td style={{ textAlign: 'right' }}>{item.data.early_leave_count}</td>
                            <td style={{ textAlign: 'right' }}>{item.data.overtime_hours}</td>
                            <td>{item.error || '-'}</td>
                          </tr>
                        ))}
                      </tbody>
        ...
        <div className="modal-footer">
          <button onClick={generateAttendanceTemplate} className="btn btn-secondary">
